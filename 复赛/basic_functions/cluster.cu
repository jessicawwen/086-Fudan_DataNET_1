    ...
    int num_nodes)
{
    ...
    int reciprocal = 0;
    int start_out_u = row_ptr_out[u];
    int end_out_u = row_ptr_out[u + 1];
    for (int i = start_out_u; i < end_out_u; ++i)
    {
        int v = col_indices_out[i];
        int start_in_v = row_ptr_in[v];
        int end_in_v = row_ptr_in[v + 1];
        int left = start_in_v;
        int right = end_in_v - 1;
        bool found = false;
        while (left <= right)
        {
            int mid = left + (right - left) / 2;
            if (col_indices_in[mid] == u)
            {
                found = true;
                break;
            }
            else if (col_indices_in[mid] < u)
            {
                left = mid + 1;
            }
            else
...
int main(int argc, char *argv[])
{
    ...
    vector<int> row_ptr_in;
    vector<int> col_indices_in;
    if (is_directed)
    {
        row_ptr_in.assign(num_nodes + 1, 0);
    ...
    cout << "Host execution time: " << fixed << setprecision(6) << host_duration.count() << " 秒\n";
    ...
    {
        extern __global__ void undirected_clustering_coefficient_csr_kernel(
            const int *__restrict__ row_ptr,
            const int *__restrict__ col_indices,
            const int *__restrict__ degree,
            double *cc,
    ...
    cout << "GPU execution time: " << fixed << setprecision(6) << gpu_duration.count() << " 秒\n";
    ...
    vector<int> sorted_node_ids = idx_to_node_id;
    sort(sorted_node_ids.begin(), sorted_node_ids.end(), [&](const int a, const int b) -> bool
         { return a > b; });

    // cout << "\n Node ID | CPU | GPU \n";
    // cout << "------------------------------------------\n";
    // for (auto it = sorted_node_ids.begin(); it != sorted_node_ids.end(); ++it)
    // {
    //     int node_id = *it;
    //     int idx = node_id_to_idx[node_id];
    //     double cpu_cc = (cc_host.find(node_id) != cc_host.end()) ? cc_host[node_id] : 0.0;
    //     double gpu_cc = cc_gpu[idx];
    //     cout << setw(7) << node_id
    //          << " | "
    //          << setw(12) << fixed << setprecision(6) << cpu_cc
    //          << " | "
    //          << setw(12) << fixed << setprecision(6) << gpu_cc << "\n";
    // }
