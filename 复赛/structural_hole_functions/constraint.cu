...
void cuda_constraint(
    ...
    cudaMalloc((void**)&d_constraints, num_nodes * sizeof(double));
    ...
    cudaMemcpy(constraints.data(), d_constraints, num_nodes * sizeof(double), cudaMemcpyDeviceToHost);
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < constraints.size(); i++) {
        std::cout << "Node " << invNodeMap[i] << ": Constraint = " << constraints[i] << std::endl;
    }
