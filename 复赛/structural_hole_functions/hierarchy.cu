...
__global__ void calculate_hierarchy(
    ...
    int in_neighbor = rowPtrOut[v + 1] - rowPtrOut[v];  
    int out_neighbor = rowPtrIn[v + 1] - rowPtrIn[v]; 
    double C = 0.0;
    double hierarchy_sum = 0.0;
    int neighbor = 0;
    double *c = new double[in_neighbor + out_neighbor]; 
    for (int i = rowPtrOut[v]; i < rowPtrOut[v + 1]; i++) {
        int w = colIdxOut[i];
        c[neighbor] = local_constraint(rowPtrOut, colIdxOut, valOut, rowPtrIn, colIdxIn, valIn, d_sum, v, w);
        C += c[neighbor];
        neighbor++;
    }
    // __syncthreads();
    for (int i = rowPtrIn[v]; i < rowPtrIn[v + 1]; i++) {
        int w = colIdxIn[i];
        c[neighbor] = local_constraint(rowPtrOut, colIdxOut, valOut, rowPtrIn, colIdxIn, valIn, d_sum, v, w);
        C += c[neighbor];
        neighbor++;
...
void cuda_hierarchy(
    ...
    cudaMalloc((void**)&d_hierarchys, num_nodes * sizeof(double));
    ...
    calculate_hierarchy<<<numBlocks, blockSize>>>(
        num_nodes,
        NODES_PER_BLOCK,
        d_rowPtrOut,
        d_colIdxOut,
    ...
    hierarchys.resize(num_nodes);
    cudaMemcpy(hierarchys.data(), d_hierarchys, num_nodes * sizeof(double), cudaMemcpyDeviceToHost);
...
int main(int argc, char** argv) {
    ...
    //     std::cout << "Node " << invNodeMap[i] << ": Hierarchy = " << hierarchys[i] << std::endl;
    // }
